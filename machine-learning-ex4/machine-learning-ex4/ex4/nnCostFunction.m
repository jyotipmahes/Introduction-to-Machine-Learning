...
J1 = (1/m)*sum(sum(-ynew.*log(z2)-(1-ynew).*log(1-z2))); 
t1 = Theta1(:,2:size(Theta1,2));
t2 = Theta2(:,2:size(Theta2,2));
 reg=lambda  * (sum( sum ( t1.^ 2 )) + sum( sum ( t2.^ 2 ))) / (2*m);
%reg= (lambda/(2*m))*(sum(sum((Theta1(:,2:input_layer_size).^2)))+ sum(sum(Theta2(:,2:hidden_layer_size).^2)));
J= J1+reg ;
...
for i=1:m
 ...
 Delta1=Delta1+ delta2(1,2:size(delta2,2))'.*(a1);
